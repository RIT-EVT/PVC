...
int main() {
    ...
    // Set up pre_charge
    IO::GPIO& key = IO::getGPIO<PVC::PVC_KEV1N::KEY_IN_PIN>(IO::GPIO::Direction::INPUT);
    IO::GPIO& batteryOne = IO::getGPIO<PVC::PVC_KEV1N::BAT_OK_1_PIN>(IO::GPIO::Direction::INPUT);
    IO::GPIO& batteryTwo = IO::getGPIO<PVC::PVC_KEV1N::BAT_OK_2_PIN>(IO::GPIO::Direction::INPUT);
    IO::GPIO& eStop = IO::getGPIO<PVC::PVC_KEV1N::ESTOP_IN_PIN>(IO::GPIO::Direction::INPUT);
    IO::GPIO& pc = IO::getGPIO<PVC::PVC_KEV1N::PC_CTL_PIN>(IO::GPIO::Direction::OUTPUT);
    IO::GPIO& dc = IO::getGPIO<PVC::PVC_KEV1N::DC_CTL_PIN>(IO::GPIO::Direction::OUTPUT);
    PVC::Contactor cont(
        IO::getGPIO<PVC::PVC_KEV1N::CONT1_PIN>(IO::GPIO::Direction::OUTPUT),
        IO::getGPIO<PVC::PVC_KEV1N::CONT2_PIN>(IO::GPIO::Direction::OUTPUT));
    IO::GPIO& apm = IO::getGPIO<PVC::PVC_KEV1N::APM_CTL_PIN>(IO::GPIO::Direction::OUTPUT);
    //    IO::GPIO& forward = IO::getGPIO<IO::Pin::PA_3>(IO::GPIO::Direction::OUTPUT);
    GFDB::GFDB gfdb(can);
    PVC::PVC_KEV1N precharge(key, batteryOne, batteryTwo, eStop, pc, dc, cont, apm, gfdb, can, MAX);
    ...
    while (1) {
        PVC::PVC_KEV1N::PVCStatus current_status = precharge.process(uart);// Update state machine

        if (current_status == PVC::PVC_KEV1N::PVCStatus::PVC_PRE_OP) {
            CONmtSetMode(&canNode.Nmt, CO_PREOP);
        } else if (current_status == PVC::PVC_KEV1N::PVCStatus::PVC_OP) {
            CONmtSetMode(&canNode.Nmt, CO_OPERATIONAL);
        }
